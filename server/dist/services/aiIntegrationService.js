...
class AiIntegrationService {
    static baseUrl = env_1.config.aiServiceUrl;
    static async predictPrice(propertyData) {
        try {
            const response = await axios_1.default.post(`${this.baseUrl}/api/predict-price`, propertyData, { timeout: 4000 });
    ...
    static async checkFraud(propertyData) {
        try {
            const response = await axios_1.default.post(`${this.baseUrl}/api/fraud-check`, propertyData, { timeout: 4000 });
            return response.data;
        }
        catch (error) {
    ...
    static async parseNaturalLanguageQuery(userPrompt) {
        try {
            const response = await axios_1.default.post(`${this.baseUrl}/api/parse-query`, { prompt: userPrompt }, { timeout: 4000 });
            return response.data;
        }
        catch (error) {
            ...
            if (lakhMatch)
                extracted.maxPrice = parseInt(lakhMatch[1], 10) * 100000;
            const crMatch = promptLower.match(/(\d+(\.\d+)?)\s*(cr|crore|crores)/);
            if (crMatch)
                extracted.maxPrice = parseFloat(crMatch[1]) * 10000000;
            if (promptLower.includes('omr'))
                extracted.locality = 'OMR';
            if (promptLower.includes('adyar'))
                extracted.locality = 'Adyar';
            if (promptLower.includes('velachery'))
                extracted.locality = 'Velachery';
            if (promptLower.includes('chennai'))
                extracted.city = 'Chennai';
            if (promptLower.includes('bangalore') || promptLower.includes('bengaluru'))
                extracted.city = 'Bangalore';
            if (promptLower.includes('parking'))
    ...
    static async getRecommendations(userId, userHistory, candidateProperties) {
        try {
            const response = await axios_1.default.post(`${this.baseUrl}/api/recommend`, {
                userId,
                userHistory,
                candidateProperties,
            }, { timeout: 5000 });
    ...
    static async chatAssistant(messages, availableProperties) {
        try {
            const response = await axios_1.default.post(`${this.baseUrl}/api/chat-assistant`, { messages, availableProperties }, { timeout: 6000 });
            return response.data;
        }
        catch (error) {
